...
MAILS="krishna.kasarla@justdial.com,yogesh.verma2@justdial.com,anurag.singh9@justdial.com,shravan.katti@justdial.com,abhishek.veeravelli@justdial.com"
#MAILS="krishna.kasarla@justdial.com"
DATE=`date "+%Y-%m-%d"`                                                                    ###Today's date 
ip_date=`date +%Y-%m-%d -d " 1 day ago" | tr '[:upper:]' '[:lower:]'`                ###Yesterday date SP format  


ERROR() { 
    if [ $? -ne 0 ] ; then
       echo -e "process failed at $DATE" |mail -r "From: <reports@justdial.com>"  -s "JD_MART_AUTO_REPORT:11.149" $EMAILID 
           sh /scripts/standard_sms_sending_script.sh -m "JD_MART_AUTO_REPORT process Failed on 11.149" -f /scripts/JD_MART_AUTO_REPORT.txt
           exit 1
        fi 
}
...
SUCCESS() { 
      if [ $? -eq 0 ] ; then
             echo "process completed successfully at $DATE" | mail -s "JD_MART_AUTO_REPORT process success:11.149" $EMAILID
             exit 0
          fi 
}
...
####STEP3:CALLING SP -call test.sp_jdmart_rating_review on 192.168.12.1.8 ###############
...
####################SUCCESS MESSAGE############################
SUCCESS
echo " JD_MART_AUTO_REPORT on 11.149" | mail -s "Process Krishna_Process   successfully completed for $PREVIOUS_DATE" ${MAILS}
