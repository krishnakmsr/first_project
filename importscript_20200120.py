...
for mast in Rec:
	...
	for master in Records: 
		...
		image_new_details='SELECT product_imagepath AS img_p,product_imagepath_fs AS imgpath_fs,product_imagepath_bsp AS imgpath_bsp,product_imagepath_tmb AS imgpath_tmb,product_imagepath_tmb_rev AS imgpath_tmb_rev,product_imagepath_tmw AS imgpath_tmw,height as hgt,width as wdth,file_name as fs,active_flag AS af,uploadedBy AS uby,updatedOn AS uon,backenduptdate AS bcupt FROM test.temp_image_new_20200131 where product_id="%s"' %(product_id)
		...
		spec_details='SELECT a.spec_id AS sid,a.spec_name AS sn,a.spec_display_name AS sdn,a.spec_display_value AS sdv,a.spec_numeric_value AS snv,a.spec_numeric_flag AS snf,a.spec_unit AS su,cast(a.spec_active_flag as signed) AS saf,cast(a.active_flag as signed) AS af, b.sort_spec_position AS ssp,b.sort_active_flag AS sort_af,a.updatedon AS uon,a.updatedby AS uby,a.backenduptdate AS bcupt  FROM (select a.spec_id,b.spec_name,b.spec_display_name,b.spec_unit,a.spec_display_value,a.spec_numeric_value,b.spec_numeric_flag,b.active_flag AS spec_active_flag,a.active_flag,a.updatedon,a.updatedby,a.backenduptdate,a.catid FROM test.tbl_spec_display a JOIN tbl_spec_master b ON a.spec_id=b.spec_id WHERE product_id="%s") a JOIN tbl_spec_mapping b ON a.spec_id=b.spec_id AND a.catid=b.catid '%(product_id)
		#print spec_details
		#sys.exit()
		...
		Mongo_Collection =  {   "pid" :product_id,
								"pn":product_name,
