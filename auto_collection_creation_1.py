...
#client = pymongo.MongoClient('mongodb://misbackend:Misbackend@321@192.168.29.53:27017')
...
cur_dt=datetime.now()
pr_dt=cur_dt.strftime("%Y-%m-%d")
#print(pr_dt)
pr_dt1=datetime.strptime(pr_dt,"%Y-%m-%d")
ti=(datetime.date(pr_dt1) + relativedelta(day=31))
#print(ti)

if(pr_dt==ti): #checking if today is lastday of the month
...
###########################################
	...
	t3=pr_dt1+timedelta(1) 
	pr_dt3=datetime.strptime(t3,"%Y-%m-%d") #first day of next month
    	monthName = pr_dt3.strftime("%b").lower()
	yearName = pr_dt3.strftime("%Y")
	month = pr_dt3.strftime("%Y%m")
    	date1=pr_dt3.strftime("%Y%m%d")
	vexpr_col5='tbl_mp_event_data_'+str(date1)
   	if(vexpr_col5 not in x1):
		db1.create_collection(vexpr_col5)
		vexpr5={
